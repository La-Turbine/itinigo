<template>
  <div class="flex min-w-[70vw] overflow-hidden rounded-3xl border border-black/10 bg-white p-5" :class="i === +$route.query.sequence! && j === +$route.query.photo! ? '!border-[#3880ff] !bg-[#3880ff22]' : ''" :data-sequence="i" :data-photo="j" @click.stop="$router.push({ query: { step: 4, sequence: i, photo: j } })">
    <div class="-m-5 mr-5 flex size-30 items-center justify-center overflow-hidden rounded-3xl rounded-r-none bg-black/1 ring ring-black/10">
    ...
    <div class="i-lucide/ellipsis-vertical -mx-1 my-1 size-5" @click.stop="$router.push({ replace: true, query: { ...$route.query, sequence: i, photo: j, action: 1 } })"></div>
  </div>
</template>
