    ...
    <ion-content forceOverscroll="false">
      <div class="flex flex-col gap-5 p-5">
        <template v-if="$route.query.mode === 'card'">
          <div class="text-center text-[2rem] text-balance">Bonjour 👋,</div>
          <div class="text-center text-[1.25rem] text-balance">Je m'appelle {{ $state.name }} et j'ai besoin de votre aide pour retrouver le fil de mon trajet.</div>
          <div class="text-center text-[1.25rem] text-balance">{{ $state.instruction || "Je comprends tout ce que vous dites si vous me parlez lentement." }}</div>
          <div class="text-center text-[1.25rem] font-semibold text-balance">Pourriez-vous m'aider à rejoindre ma destination s'il vous plait ?</div>
          <div class="text-center text-[1.25rem] font-semibold text-balance">Merci beaucoup.</div>
          <hr class="w-1/2 border-t border-black/50" />
        ...
        <template v-if="!$route.query.mode">
          <div class="flex max-w-md gap-5 rounded-lg border border-black/15 bg-white p-5" @click="help">
          ...
          <div class="flex max-w-md gap-5 rounded-lg border border-black/15 bg-white p-5" @click="map">
            <div class="w-[85px]">
              <!-- prettier-ignore -->
          ...
          <div class="flex max-w-md gap-5 rounded-lg border border-black/15 bg-white p-5" @click="call" v-if="$state.phone">
            <div class="text-[50px]">📞</div>
