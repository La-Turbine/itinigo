<template>
  <ion-page>
    ...
    <ion-content>
      <div v-if="!currentChoice">BRAVOOOO</div>
      <!-- https://play.tailwindcss.com/90GckuhEBW -->
      <div style="width: 100vw; overflow-x: auto" v-else-if="currentChoice.stops">
      ...
      <div style="display: flex; flex-direction: column; height: 100%" v-else>
        <div style="display: flex; height: 85%">
          <img style="max-width: 100%; max-height: 100%; height: auto; margin: auto" :src="$state.photos[currentChoice.id]" />
        </div>
...
watch(
  ...
  (number) => {
    ...
    watch(
      ...
      (percentage) => notify("Préparez-vous à descendre, quand les portes s'ouvriront"),
      { once: true }
    )
    watch(
      () => progress.value.percentage < 0.9,
      (percentage) => $router.push({ query: { step: currentStep.value + 1 } }),
      { once: true }
    )
  }
)
